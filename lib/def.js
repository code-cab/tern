...
(function(mod) {
  ...
  function isSimpleAnnotation(spec) {
    ...
    for (var prop in spec)
      if (prop != "!type" && prop != "!doc" && prop != "!url" && prop != "!span" &&
          prop != "!data" && prop != '!default' && prop != '!ctor')
        return false;
    return true;
  }
  ...
  function passOne(base, spec, path) {
    if (!base) {
      ...
      if (tp) {
        ...
        // else if (tp === 'string') base = infer.cx().str;
        else {
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      if (typeof inner == "string") {
        ...
        if (inner["!data"]) known.metaData = inner["!data"];
        if (inner["!default"]) known.default = inner["!default"];
        if (inner["!ctor"]) known.ctor= inner["!ctor"];
      }
    }
    return base;
  }
  ...
  function copyInfo(spec, type) {
    ...
    if (spec["!default"]) type.ctor = spec["!default"];
    if (spec["!ctor"]) type.ctor= spec["!ctor"];
  }
