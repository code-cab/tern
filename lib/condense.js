...
(function(root, mod) {
  ...
  function reach(type, path, id, state, byName) {
    var actual, orig, relevant, newPath, oldPath;
    ...
    for (var i = 0; i < types.length; i += 1) {
      ...
      if (shorter) {
          ...
          if (actual.reached(newActualPath, state, !relevant) && relevant) {
              ...
              data.ctor = type._ctor || (actual != type && state.isTarget(actual.origin) && actual._ctor) || data._ctor;
              data.data = type.metaData || actual.metaData;
              // Added for CodeCab
              if (type.default !== undefined) data.default = type.default;
  ...
  infer.Fn.prototype.reached = function(path, state, concrete) {
    // if (this.proto && this.proto.hasCtor) {
    //   if (!this.proto.hasCtor.path) {
    //       reach(this.proto.hasCtor, null, this.proto.hasCtor.name, state);
    //   }
    //   this._ctor = this.proto.hasCtor.path;
    // }
